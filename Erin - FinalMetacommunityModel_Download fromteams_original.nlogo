...
globals [ years output-file-name ]  ; Add this line here to declare the global variable
; new code added on Oct 26th, 2024 to sort species diversity on unique patches in a descending order.
...
to calculate-and-export-turnover
  ask patches [
    ; Save the current species list before updating
    if ticks > 1 [  ; Ensure that we don't overwrite the list right after setup
      set previous-species-list current-species-list
    ]

    ; Update the current species list with unique species present
    set current-species-list remove-duplicates [species] of turtles-here

    ; Calculate turnover (species lost + species gained)
    let shared-species filter [species-item -> member? species-item previous-species-list] current-species-list
    let lost-species length previous-species-list - length shared-species
    let gained-species length current-species-list - length shared-species
    let turnover lost-species + gained-species

    ; Collect the specialist-or-generalist information for all turtles on the patch
    let specialization-types [specialist-or-generalist] of turtles-here

    ; Export turnover data to the CSV file
    if ticks mod 50 = 0 [
      file-open output-file-name
      file-print (word years ", " pxcor ", " pycor ", " previous-species-list ", " current-species-list ", " turnover ", " specialization-types)
      file-close
    ]
  ]
end
...
to setup
  ...
  file-open output-file-name
  file-print "Year, Patch X, Patch Y, Previous Species List, Current Species List, Turnover, Specialization"
  file-close
...
to go
  ...
  ; Update and log patch diversity every 50 ticks
  update-and-log-patch-species-diversity

  ; Calculate and export turnover for patches
    ...
    [ (ifelse
      ...
      [
        ...
        if food <= 0 [ die ]
          ...
          let K (max-agents-on-a-patch * 49 * fraction) * specialist-K
...
<experiments>
  ...
  <experiment name="Data collection 1 - testing" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
